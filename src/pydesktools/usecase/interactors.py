...
class PluginUseCase:
    ...
    def __search_for_plugins_in(self, plugins_path: List[str], package_name: str):
        for directory in plugins_path:
            ...
            if entry_point is not None:
                ...
                # Importing the module will cause IPluginRegistry to invoke it's __init__ function
    ...
    def discover_plugins(self, reload: bool):
        """
        Discover the plugin classes contained in Python files, given a list of directory names to scan.
        """
        if reload:
            self.modules.clear()
            IPluginRegistry.plugin_registries.clear()
    ...
    @staticmethod
    def registry_plugin(module: type, logger: Logger) -> PluginCore:
        """Create a plugin instance from the given module

        Args:
            module (type): module to initialize
            logger (Logger): logger for the module to use

        Returns:
            PluginCore: a high level plugin
        """
