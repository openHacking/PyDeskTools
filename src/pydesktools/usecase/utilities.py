...
from model import DependencyModule, PluginConfig
...
class PluginUtility:
    ...
    def filter_plugins_paths(plugins_package) -> List[str]:
        ...
        Args:
            plugins_package (_type_): plugins package
    ...
    @staticmethod
    def __get_missing_packages(installed: List[Distribution], required: Optional[List[DependencyModule]]) -> List[DependencyModule]:
        missing = list()
    ...
    def __read_configuration(self, module_path) -> Optional[PluginConfig]:
        try:
            ...
            self._logger.error(
                'Unable to parse plugin configuration to data class', e)
        return None
    ...
    def setup_plugin_configuration(self, package_name, module_name) -> Optional[str]:
        ...
        Args:
            package_name (_type_): package of the potential plugin
            module_name (_type_): module of the potential plugin

        Returns:
            Optional[str]: a module name to import
        """
